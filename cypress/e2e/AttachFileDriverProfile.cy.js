...
describe('Upload a file', { testIsolation: false }, () => {
    ...
    it('should upload a file for a driver', () => {
        ...
        cy.get('.upload--btn__label').selectFile("cypress/fixtures/sample.pdf");
        cy.get('#uploadFileWithConfirmation').click()
        cy.get('.person-details__attachments__name').should('be.visible').contains('sample.pdf')
        cy.get('.samba-toggle > .v-btn > .v-btn__content > .v-icon').click()
        cy.get('[data-cy="form-dialog-save-button"]').click()
